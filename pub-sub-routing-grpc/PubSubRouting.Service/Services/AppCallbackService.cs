...
namespace PubSubRouting.Service.Services
{
    public class AppCallbackService : AppCallback.AppCallbackBase
    {
        ...
        public async override Task<TopicEventResponse> OnTopicEvent(TopicEventRequest request, ServerCallContext context)
        {
            ...
            var dataType = TypeCache.GetType(request.Type);// must be AssemblyQualifiedName of Type, but may use simple Name of Assembly (without the Version, Culture and PublicKeyToken) instead of FullName of Assembly (with the Version, Culture and PublicKeyToken)
            var data = JsonSerializer.Deserialize(request.Data.Span, dataType, _daprClient.JsonSerializerOptions);
